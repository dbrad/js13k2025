import { EULER, _max, _min, _round } from "./math";
...
export let updateCamera = (x: number, y: number, delta: number): void => {
    ...
    cameraTarget[X] = _max(minX, _min(maxX, cameraTarget[X]));
    cameraTarget[Y] = _max(minY, _min(maxY, cameraTarget[Y]));
    ...
    cameraPos[X] = _round(vCameraPos[X]);
    cameraPos[Y] = _round(vCameraPos[Y]);
};
