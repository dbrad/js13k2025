...
let textureDefinitions: TextureDefinition[] = [
  ...
  [TEXTURE_TYPE_SPRITE_STRIP, [TEXTURE_C_16x16, TEXTURE_D_PAD, TEXTURE_D_PAD_UP, TEXTURE_D_PAD_RIGHT, TEXTURE_A_BUTTON_UP, TEXTURE_B_BUTTON_UP, TEXTURE_A_BUTTON_DOWN, TEXTURE_B_BUTTON_DOWN], 16, 16, 16, 16],
];
...
export let loadTextureAtlas = async (): Promise<void> => {
  return new Promise(async (resolve) => {
    ...
    assert(ATLAS_HEIGHT === imageBitmap.height, `ATLAS HEIGHT CHANGED (expected: ${ATLAS_HEIGHT} actual: ${imageBitmap.height})`);
